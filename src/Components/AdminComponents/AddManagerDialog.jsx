...
const AddManagerDialog = ({ managerType, setIsUpdated }) => {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, action) => {
      try {
        ...
        const endpoint = managerType === 'warehouse' 
          ? `${baseURL}/api/v1/admin/users/warehouse/add`
          : `${baseURL}/api/v1/admin/users/shipment/add`;

        // ✅ Send as JSON instead of FormData to match backend requirements
        const requestData = {
          fullName: values.name, // ✅ Backend expects 'fullName'
        ...
        Swal.fire({
          title: 'Success!',
          text: `${managerType === 'warehouse' ? 'Warehouse' : 'Shipment'} Manager Added Successfully!`,
          icon: 'success',
        });
              ...
              <div className="space-y-4">
                ...
                <div className="mt-4 text-right">
                  <button 
                    ...
                    disabled={loading}
                  >
                    {loading ? <CircularProgress size={24} color="inherit" /> : `Add ${managerType === 'warehouse' ? 'Warehouse' : 'Shipment'} Manager`}
                  </button>
                </div>
              </div>
            </form>
