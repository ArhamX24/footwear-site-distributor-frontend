...
const ProductCard = ({ product, setPlaceOrderModal, setSelectedProductDetails, variant}) => { 
  ...
  return ( 
    <div className="w-full bg-white rounded-lg md:rounded-xl shadow-sm md:shadow-md overflow-hidden relative flex flex-col h-full hover:shadow-lg md:hover:shadow-xl transition-all duration-300 border border-gray-100"> 
      ...
      <div className="relative w-full aspect-[3/4] md:aspect-[4/5] overflow-hidden bg-gray-100/90"> 
        <img 
          src={product.images[currentImageIndex]} 
          alt={product.name} 
          className="w-fit h-fit object-cover object-top cursor-pointer transition-transform duration-500 hover:scale-105 md:hover:scale-110" 
      ...
      {isZoomed && ( 
        ...
        > 
            ...
            <button 
                onClick={handlePrev} 
                className="absolute left-1 md:left-2 lg:-left-12 bg-white/20 hover:bg-white/40 text-white p-2 md:p-3 rounded-full cursor-pointer transition-colors backdrop-blur-md z-10 text-sm md:text-base" 
            > 
                ❮ 
            </button> 
            ...
            <img 
                src={product.images[currentImageIndex]} 
                alt="Zoomed View" 
                className="max-w-full max-h-full object-contain rounded-md md:rounded-lg shadow-2xl" 
            /> 
        
            {/* Right Arrow Button */} 
            <button 
                onClick={handleNext} 
                className="absolute right-1 md:right-2 lg:-right-12 bg-white/20 hover:bg-white/40 text-white p-2 md:p-3 rounded-full cursor-pointer transition-colors backdrop-blur-md z-10 text-sm md:text-base" 
            > 
                ❯ 
            </button> 
            ...
            <button 
                id="close-btn" 
                className="absolute top-1 right-1 md:top-2 md:right-2 lg:-top-10 lg:-right-10 bg-white/20 hover:bg-red-500 text-white p-2 rounded-full transition-colors backdrop-blur-md text-sm md:text-base" 
                onClick={handleCloseZoom} 
            > 
                ✕ 
            </button> 
