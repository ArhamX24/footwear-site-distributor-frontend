...
const AddContractorDialog = ({ setIsUpdated }) => {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, action) => {
      try {
        ...
        // ✅ Send as JSON instead of FormData since your controller expects req.body fields
        const requestData = {
          fullName: values.name, // ✅ Controller expects 'fullName'
          phoneNo: values.phoneNo,
          password: values.password
        };

        const response = await axios.post(
          `${baseURL}/api/v1/admin/users/contractor/add`,
              ...
              <div className="space-y-4">
                ...
                <div className="mt-4 text-right">
                  <button 
                    ...
                    disabled={loading}
                  >
                    {loading ? <CircularProgress size={24} color="inherit" /> : "Add Contractor"}
                  </button>
                </div>
              </div>
            </form>
